...
func (r *RabbitMQ) PublishMessage(ctx context.Context, routingKey string, message contracts.AmqpMessage) error {
	...
	return r.Channel.PublishWithContext(ctx, TripExchange, routingKey, false, false, amqp.Publishing{
...
func (r *RabbitMQ) setupExchangesAndQueues() error {
	...
	if err := r.declareAndBindQueue(FindAvailableDriversQueue, []string{contracts.TripEventCreated, contracts.TripEventDriverNotInterested}, TripExchange); err != nil {
		return fmt.Errorf("failed to declare and bind queue %s: %v", FindAvailableDriversQueue, err)
	}
	if err := r.declareAndBindQueue(DriverCmdTripRequestQueue, []string{contracts.DriverCmdTripRequest}, TripExchange); err != nil {
		return fmt.Errorf("failed to declare and bind queue %s: %v", DriverCmdTripRequestQueue, err)
	}
...
func (r *RabbitMQ) declareAndBindQueue(queueName string, messageTypes []string, exchange string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to declare a queue %s: %v", queueName, err)
	}
	...
	for _, msg := range messageTypes {
		if err := r.Channel.QueueBind(
			...
			return fmt.Errorf("failed to bind a queue %s to an exchange %s with routing key %s: %v", queueName, exchange, msg, err)
		}
	}
	return nil
}
